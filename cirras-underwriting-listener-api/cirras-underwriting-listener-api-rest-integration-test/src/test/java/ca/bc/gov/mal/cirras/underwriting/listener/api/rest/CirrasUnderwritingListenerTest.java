package ca.bc.gov.mal.cirras.underwriting.listener.api.rest;
...
public class CirrasUnderwritingListenerTest {

	private static final Logger logger = LoggerFactory.getLogger(CirrasUnderwritingListenerTest.class);
	...
	private static CirrasUnderwritingListenerService service;
	...
	public static void beforeClass() throws Exception {
		...
		String restContext = properties.getProperty("context.wfhr.diary-submission-listener-rest");
		Assert.assertNotNull("'context.wfhr.diary-submission-listener-rest' is a required property", restContext);
...

		{
			...
			AuthorizationCodeService authorizationCodeService = new AuthorizationCodeServiceImpl(clientId,
			...
			AccessToken token;
			{
				...
				token = authorizationCodeService.getImplicitToken(scope, redirectUri, organizationId,
						siteMinderUserType, siteMinderUserIdentifier, siteMinderAuthoritativePartyIdentifier, username,
						secret);
			}

			service = new CirrasUnderwritingListenerServiceImpl("Bearer " + token.getAccessToken());
