...
public class SwaggerTests extends EndpointsTest {
	...
	protected static final String scope = getScopeString(Scopes.GET_TOP_LEVEL);
	
	private static ObjectMapper mapper = new ObjectMapper();

	//@Test
	public void testSwagger() throws RestClientServiceException, IOException  {
		...
		for(String pathKey:paths.keySet()) {
			...
			for(String methodKey:path.keySet()) {
				...
				//Assert.assertNotNull("missing description", method.get("description"));
				//Assert.assertFalse("missing description", method.get("description").toString().isEmpty());
...
//					if("put".equalsIgnoreCase(methodKey)||"delete".equalsIgnoreCase(methodKey)) {
//						Assert.assertTrue("missing If-match parameter", foundIfMatch);
//					}
				}
