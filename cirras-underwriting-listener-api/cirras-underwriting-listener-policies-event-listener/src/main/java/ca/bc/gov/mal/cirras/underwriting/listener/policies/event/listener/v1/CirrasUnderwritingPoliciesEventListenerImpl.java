...
public class CirrasUnderwritingPoliciesEventListenerImpl implements EventListener, Runnable {

	private static final Logger logger = LoggerFactory.getLogger(CirrasUnderwritingPoliciesEventListenerImpl.class);
	...
	private CirrasUnderwritingPoliciesListenerService cirrasUnderwritingPoliciesListenerService;
	...
	public CirrasUnderwritingPoliciesEventListenerImpl() {
		logger.debug("<CirrasUnderwritingPoliciesEventListenerImpl");
	...
	public String getProcessName() {
		...
		return "UNDERWRITING_POLICIES_EVENT_PROCESSOR";
	}
			...
			while (running) {
				...
				try {
					...
					if(message!=null) {
						...
						String requestId = "CIRRASUNDERWRITINGPOLICIESLISTENER" + message.getJMSMessageID();
	...
	private void processMessage(String message, int jmsXDeliveryCount, int maximumRedeliveries, String messageId) throws Throwable {
		...
		try {
			...
			cirrasUnderwritingPoliciesListenerService.processCirrasUnderwritingPoliciesEvent(message, messageId, factoryContext);
		}
		catch (ServiceException e) {
			
			String errorMessage = "Saw " + e.getClass().getName() + " while processing cirras underwriting policies updates (ServiceException)"
			...
			String errorMessage = "Saw " + e.getClass().getName() + " while processing cirras underwriting policies updates (Throwable)"
					+ (e.getMessage() != null ? ": reason " + e.getMessage() : "");
			logger.error(errorMessage, e);
	...
	public void setCirrasUnderwritingPoliciesListenerService(CirrasUnderwritingPoliciesListenerService cirrasUnderwritingPoliciesListenerService) {
		this.cirrasUnderwritingPoliciesListenerService = cirrasUnderwritingPoliciesListenerService;
	}
